...
impl CapabilityCatalog {
    ...
    pub fn format(&self, id: &str) -> Option<&FormatCapabilityProfile> {
        ...
        self.formats.iter().find(|profile| {
            profile.id == normalized || profile.extensions.iter().any(|extension| extension == &normalized)
        })
    }
...
fn default_format_capabilities() -> Vec<FormatCapabilityProfile> {
    ...
    let image_actions = vec![
        ...
        "image-rotate-left", "image-rotate-right", "image-rotate-180", "image-rotate",
        "image-flip-horizontal", "image-flip-vertical", "image-auto-orient",
        "image-auto-enhance", "image-adjust", "image-grayscale", "image-sepia",
        "image-sharpen", "image-blur", "image-noise-reduce", "image-threshold",
        "image-posterize", "image-pixelate", "image-flatten", "image-trim",
        "image-crop-center", "image-resize-exact", "image-border", "image-vignette",
        "image-watermark", "strip-metadata", "extract-metadata", "ocr-image",
        "images-to-pdf", "zstd-compress", "lz4-compress",
    ];
    let pdf_actions = vec![
        "pdf-merge", "pdf-split", "pdf-reorder", "pdf-rotate-pages", "pdf-select-pages",
        "pdf-linearize", "pdf-optimize-lossless", "pdf-repair", "pdf-flatten-rotation",
        "pdf-flatten-annotations", "pdf-compress", "pdf-ocr", "pdf-to-images", "pdf-extract-text", "strip-metadata", "extract-metadata",
        "zstd-compress", "lz4-compress",
    ];
    let video_actions = vec![
        "video-convert", "video-rotate", "video-resize", "video-mute", "video-thumbnail",
        "media-trim", "media-compatible", "media-compress", "extract-audio", "video-to-gif", "strip-metadata", "extract-metadata", "zstd-compress",
        "lz4-compress",
    ];
    let audio_actions = vec![
        "audio-convert", "audio-normalize", "audio-gain", "audio-mono", "media-trim",
        "media-compatible", "media-compress", "strip-metadata",
        "extract-metadata", "zstd-compress", "lz4-compress",
    ];

    vec![
        format_profile("jpeg", "JPEG / JPG", Image, &["jpg", "jpeg"], true, &image_actions, &["png", "webp", "avif", "heic", "jxl", "tiff", "gif", "pdf"], &["zst", "lz4", "zip", "7z"]),
        format_profile("png", "PNG", Image, &["png"], true, &image_actions, &["jpg", "webp", "avif", "heic", "jxl", "tiff", "gif", "pdf"], &["zst", "lz4", "zip", "7z"]),
        format_profile("webp", "WebP", Image, &["webp"], true, &image_actions, &["jpg", "png", "avif", "tiff", "gif", "pdf"], &["zst", "lz4", "zip", "7z"]),
        format_profile("heic", "HEIC / HEIF", Image, &["heic", "heif"], true, &image_actions, &["jpg", "png", "webp", "avif", "tiff", "pdf"], &["zst", "lz4", "zip", "7z"]),
        format_profile("avif", "AVIF", Image, &["avif"], true, &image_actions, &["jpg", "png", "webp", "tiff", "pdf"], &["zst", "lz4", "zip", "7z"]),
        format_profile("tiff", "TIFF", Image, &["tif", "tiff"], true, &image_actions, &["jpg", "png", "webp", "avif", "pdf"], &["zst", "lz4", "zip", "7z"]),
        format_profile("raw", "RAW photo", Image, &["dng", "cr2", "cr3", "nef", "arw", "orf", "raf", "rw2", "pef"], true, &["image-convert", "image-auto-orient", "image-auto-enhance", "image-resize-exact", "strip-metadata", "extract-metadata", "zstd-compress", "lz4-compress"], &["jpg", "png", "tiff", "webp", "avif"], &["zst", "lz4", "zip", "7z"]),
        format_profile("pdf", "PDF", Pdf, &["pdf"], true, &pdf_actions, &["png", "txt"], &["zst", "lz4", "zip", "7z"]),
        format_profile("docx", "Word / DOCX", Document, &["doc", "docx", "odt", "rtf"], false, &["office-to-pdf", "office-convert", "zstd-compress", "lz4-compress"], &["pdf"], &["zst", "lz4", "zip", "7z"]),
        format_profile("xlsx", "Excel / tableur", Spreadsheet, &["xls", "xlsx", "xlsm", "ods", "csv", "tsv"], false, &["office-to-pdf", "office-convert", "zstd-compress", "lz4-compress"], &["pdf"], &["zst", "lz4", "zip", "7z"]),
        format_profile("pptx", "PowerPoint / présentation", Presentation, &["ppt", "pptx", "odp"], false, &["office-to-pdf", "office-convert", "zstd-compress", "lz4-compress"], &["pdf"], &["zst", "lz4", "zip", "7z"]),
        format_profile("text", "Texte / Markdown / HTML", Text, &["txt", "md", "html", "rst", "json", "xml", "yaml", "toml"], true, &["text-to-pdf", "text-convert", "zstd-compress", "lz4-compress"], &["pdf", "html", "md", "docx", "epub", "txt"], &["zst", "lz4", "zip", "7z"]),
        format_profile("epub", "EPUB / ebook", Ebook, &["epub", "fb2"], false, &["ebook-convert", "zstd-compress", "lz4-compress"], &["html", "md", "docx", "txt", "epub"], &["zst", "lz4", "zip", "7z"]),
        format_profile("audio", "Audio", Audio, &["mp3", "wav", "aac", "m4a", "flac", "ogg", "opus", "aiff"], false, &audio_actions, &["mp3", "m4a", "wav", "flac", "ogg", "opus"], &["zst", "lz4", "zip", "7z"]),
        format_profile("video", "Vidéo", Video, &["mp4", "mov", "mkv", "avi", "webm", "mpeg", "wmv", "flv", "m2ts"], false, &video_actions, &["mp4", "webm", "mkv", "mov", "gif", "m4a", "mp3"], &["zst", "lz4", "zip", "7z"]),
        format_profile("archive", "Archive", Archive, &["zip", "7z", "rar", "tar", "gz", "bz2", "xz", "zst", "lz4"], false, &["archive-extract", "archive-repack", "zstd-decompress", "lz4-decompress"], &["zip", "7z", "tar", "tar.zst", "tar.lz4"], &[]),
    ]
}
...
mod tests {
    ...
    fn format_matrix_exposes_deep_image_tooling() {
        ...
        assert!(jpeg.actions.iter().any(|action| action == "image-watermark"));

...
pub fn preview_batch_rename(
    ...
    Ok(RenamePreview { items, total, changed, conflicts, truncated })
}
...
pub async fn apply_batch_rename(
    ...
    if items.iter().any(|item| item.conflict) {
        return Err("Des collisions sont apparues depuis l’aperçu. Régénérez-le avant d’appliquer.".into());
    }
    ...
    Ok(FileOperationResult { processed, destination: None, warnings: Vec::new() })
}
...
pub fn preview_organization(
    ...
    let truncated = total > PREVIEW_ITEM_LIMIT;
    items.truncate(PREVIEW_ITEM_LIMIT);
    Ok(OrganizationPreview { items, total, truncated, categories })
}
...
pub async fn apply_organization(
    ...
    Ok(FileOperationResult { processed, destination: Some(request.destination_root), warnings: Vec::new() })
}
...
fn build_rename_plan(
    ...
    for asset in assets.into_iter().filter(|asset| !matches!(asset, Asset::Directory(_))) {
        ...
        let stem = source.file_stem().and_then(|value| value.to_str()).unwrap_or(&common.name);
        let date = common.modified_at.unwrap_or_else(Utc::now);
        let parent_name = parent.file_name().and_then(|value| value.to_str()).unwrap_or_default();
        let mut name = render_name(rule, stem, extension, parent_name, date, counter);
        counter = counter.saturating_add(1);
        if rule.preserve_extension && !extension.is_empty() && Path::new(&name).extension().is_none() {
            name.push('.');
        ...
        items.push(RenamePreviewItem {
            ...
            warning: conflict.then(|| {
                if duplicate_target { "Deux éléments produisent le même nom.".into() }
                else { "Un élément existe déjà avec ce nom.".into() }
            }),
        });
    }
    Ok(items)
    ...
    for asset in assets.into_iter().filter(|asset| !matches!(asset, Asset::Directory(_))) {
        ...
        *categories.entry(category.clone()).or_insert(0) += 1;
        ...
        if mode == "typeDate" || mode == "date" {
            if mode == "date" { directory = destination_root.to_path_buf(); }
            directory = directory.join(format!("{:04}", date.year())).join(format!("{:02}", date.month()));
        }
        let (target, resolved) = reserve_incremented_target(directory.join(&common.name), &mut reserved);
...
pub async fn duplicate_cleanup_plan(
    ...
    require_active_session(&state)?;
    let report = duplicate_report(&state, request.workspace_id, &request.selected_asset_ids).await?;
...
pub async fn quarantine_duplicates(
    ...
    let destination = request.destination.or_else(|| {
        state
            .storage
    ...
    }).ok_or_else(|| "Configurez un dossier FileFlow ou choisissez une destination de quarantaine.".to_owned())?;
    let destination_for_worker = destination.clone();
    let result = tokio::task::spawn_blocking(move || quarantine_files(sources, &destination_for_worker))
        .await
...
async fn duplicate_report(
    ...
    let _lease = scheduler
        .acquire("native-duplicates", ResourceProfile {
            cpu_weight: 4,
            memory_mb: 256,
            io_weight: 3,
...
fn build_duplicate_plan(report: DuplicateReport, strategy: DuplicateCleanupStrategy) -> DuplicateCleanupPlan {
    let warnings = report.warnings.into_iter().map(|warning| format!("{} : {}", warning.path.display(), warning.message)).collect();
    ...
    for group in report.confirmed_groups {
        ...
        let Some(keep) = assets.first().cloned() else { continue; };
    ...
    let quarantine_count = groups.iter().map(|group| group.quarantine_asset_ids.len()).sum();
    DuplicateCleanupPlan { groups, reclaimable_bytes, quarantine_count, warnings }
}

fn duplicate_order(left: &Path, right: &Path, strategy: DuplicateCleanupStrategy) -> std::cmp::Ordering {
    match strategy {
        DuplicateCleanupStrategy::ShortestPath => left.components().count().cmp(&right.components().count()).then_with(|| left.cmp(right)),
        DuplicateCleanupStrategy::Newest => modified_key(right).cmp(&modified_key(left)).then_with(|| left.cmp(right)),
        DuplicateCleanupStrategy::Oldest => modified_key(left).cmp(&modified_key(right)).then_with(|| left.cmp(right)),
    }
}
...
fn transactional_rename(operations: Vec<(PathBuf, PathBuf)>) -> Result<usize, String> {
    if operations.is_empty() { return Ok(0); }
    ...
    for (source, target) in &operations {
        ...
        let parent = source.parent().ok_or_else(|| format!("Chemin source invalide : {}", source.display()))?;
    ...
    for index in 0..staged.len() {
        ...
        if let Err(error) = fs::rename(temporary, target) {
            ...
            return Err(format!("Impossible de finaliser {} : {error}", target.display()));
        }
        finalized += 1;
    }
    Ok(finalized)
...
fn rollback_finalized(staged: &[(PathBuf, PathBuf, PathBuf)], failed_index: usize) {
    ...
    for (source, temporary, _) in staged[failed_index..].iter().rev() {
        if temporary.exists() { let _ = fs::rename(temporary, source); }
    }
}
...
fn move_operations(operations: Vec<(PathBuf, PathBuf)>) -> Result<usize, String> {
    ...
    for (source, target) in operations {
        ...
        if let Err(error) = move_file_safe(&source, &target) {
            ...
            return Err(format!("Impossible de déplacer {} : {error}", source.display()));
        }
...
fn quarantine_files(sources: Vec<PathBuf>, destination: &Path) -> Result<(usize, Vec<String>), String> {
    fs::create_dir_all(destination).map_err(|error| error.to_string())?;
    let mut reserved = HashSet::new();
    let mut warnings = Vec::new();
    let mut processed = 0usize;
    for source in sources {
        let name = source.file_name().and_then(|value| value.to_str()).unwrap_or("doublon");
...
fn render_name(rule: &RenameRule, stem: &str, extension: &str, parent: &str, date: DateTime<Utc>, counter: u64) -> String {
    let mut base = if rule.search.is_empty() { stem.to_owned() } else { stem.replace(&rule.search, &rule.replace) };
    ...
    let counter_text = format!("{:0width$}", counter, width = rule.counter_padding.clamp(1, 12));
    rule.template
        .replace("{name}", &base)
...
fn title_case(value: &str) -> String {
    value
        ...
        .map(|part| {
            ...
            characters.next().map(|first| first.to_uppercase().collect::<String>() + &characters.as_str().to_lowercase()).unwrap_or_default()
        })
        .collect::<Vec<_>>()
        .join(" ")
}
...
fn sanitize_filename(value: &str) -> String {
    ...
    if trimmed.is_empty() { "fichier".into() } else { trimmed.chars().take(240).collect() }
}
...
fn reserve_incremented_target(base: PathBuf, reserved: &mut HashSet<PathBuf>) -> (PathBuf, bool) {
    ...
    while target.exists() || !reserved.insert(normalized_path_key(&target)) {
        ...
        let stem = base.file_stem().and_then(|value| value.to_str()).unwrap_or("fichier");
...
fn default_rename_template() -> String { "{name}-{counter}".into() }
fn default_counter_start() -> u64 { 1 }
fn default_counter_padding() -> usize { 3 }
fn default_true() -> bool { true }
fn default_organization_mode() -> String { "type".into() }

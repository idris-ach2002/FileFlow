        ...
        @switch (currentStep()) {
            ...
            <section class="stage-card action-stage">
              ...
              <div class="source-strip">
                @for (asset of visibleSourceAssets(); track asset.data.id) {
                  <article><span class="file-badge" [attr.data-family]="assetFamily(asset)">{{ assetMark(asset) }}</span><div><strong>{{ asset.data.name }}</strong><small>{{ assetFormat(asset) }} · {{ assetSize(asset) }}</small></div></article>
                }
              ...
              <div class="configure-layout">
                ...
                <aside class="preview-panel">
                  ...
                  @if (pdfPreviewUrl(); as url) {
                    ...
                    <div class="image-preview"><img [src]="image" alt="Aperçu du fichier" /></div>
                  } @else {
          ...
          @case (4) {
            <section class="stage-card treatment-card">
              ...
              <div class="timeline">
                <div class="done"><span>✓</span><div><strong>Préparation</strong><small>Fichiers analysés et chemin choisi</small></div></div>
                <div class="active"><span></span><div><strong>Traitement</strong><small>{{ store.executionCompleted() }} / {{ store.executionTotal() || selectionCount() }} élément(s)</small></div></div>
                <div><span></span><div><strong>Finalisation</strong><small>Validation puis nettoyage des intermédiaires</small></div></div>
              </div>

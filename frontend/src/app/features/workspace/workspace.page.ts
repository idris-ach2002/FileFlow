        ...
        <div class="workspace-layout" [class.action-open]="activeAction()">
            ...
            @if (store.insights(); as insights) {
              <section class="insight-panel ff-card">
                ...
                @if (insights.duplicateSizeCandidates.length) {
                  <div class="duplicate-hint"><span class="duplicate-icon">≋</span><div><strong>{{ insights.duplicateSizeCandidates.length }} groupes à vérifier</strong><small>Jusqu’à {{ formatBytes(insights.potentialDuplicateBytes) }} potentiellement récupérables. Un hash confirmera les vrais doublons.</small></div></div>
                }
          ...
          @if (activeAction(); as action) {
            <aside class="action-drawer ff-card">
              ...
              <div class="drawer-status" [class.warning]="!capabilities.isActionReady(action)"><span>{{ capabilities.isActionReady(action) ? '●' : '!' }}</span><div><strong>{{ capabilities.isActionReady(action) ? 'Moteurs disponibles' : 'Dépendance manquante' }}</strong><small>{{ engineStatus(action) }}</small></div></div>
              <div class="drawer-section"><label>Portée</label><div class="scope-card"><strong>{{ store.selectedCount() ? store.selectedCount() + ' sélectionné(s)' : 'Tout le groupe compatible' }}</strong><span>{{ action.batchable ? 'Traitement par lot activé' : 'Une opération à la fois' }}</span></div></div>
              <div class="drawer-section"><label>Destination du résultat</label><div class="segmented"><button type="button" [class.active]="destination() === 'subfolder'" (click)="destination.set('subfolder')">Sous-dossier</button><button type="button" [class.active]="destination() === 'same'" (click)="destination.set('same')">Même dossier</button><button type="button" [class.active]="destination() === 'choose'" (click)="destination.set('choose')">Choisir</button></div></div>
              <div class="drawer-section"><label>Protection</label><div class="safety-list"><span><b>✓</b> Originaux conservés</span><span><b>✓</b> Nom de conflit automatique</span><span><b>✓</b> Fichier temporaire puis finalisation atomique</span></div></div>
              <div class="drawer-footer"><button class="ff-button secondary" type="button" (click)="store.closeAction()">Annuler</button><button class="ff-button" type="button" [disabled]="!capabilities.isActionReady(action)" (click)="prepareAction(action)">Préparer l’opération</button></div>
...
export class WorkspacePage {
  ...
  protected selectAndSuggest(asset: Asset): void {
    ...
    if (candidate) this.store.openAction(candidate.id);
  }

  protected openAction(action: ActionDescriptor): void { this.actionNotice.set(null); this.store.openAction(action.id); }
  ...
  protected prepareAction(action: ActionDescriptor): void {
    const count = this.store.selectedCount() || this.store.recommendations().find((item) => item.actionId === action.id)?.affectedAssets || 1;
    this.actionNotice.set(`Configuration prête pour ${count} élément${count > 1 ? 's' : ''}. Le moteur d’exécution utilisera la destination « ${this.destinationLabel()} » sans modifier les originaux.`);
  }
  ...
  private destinationLabel(): string { return this.destination() === 'subfolder' ? 'Sous-dossier FileFlow' : this.destination() === 'same' ? 'Même dossier' : 'Dossier à choisir'; }
}

...
@Component({
    ...
    <div class="home-shell">
      ...
      <section class="action-grid" aria-label="Actions rapides">
        @for (action of featuredActions(); track action.id) {
          <article class="action-card ff-card">
            <button class="action-launch" type="button" (click)="startAction(action)">
              ...
              <span class="action-state" [class.missing]="capabilities.actionState(action) !== 'ready'">
                {{ capabilities.actionState(action) === 'ready' ? 'Prêt' : capabilities.actionState(action) === 'missing-engine' ? 'À installer' : 'Planifié' }}
              </span>
            </button>
            <button class="favorite-button" type="button" [class.active]="capabilities.isFavorite(action.id)" [attr.aria-label]="capabilities.isFavorite(action.id) ? 'Retirer des favoris' : 'Ajouter aux favoris'" (click)="toggleFavorite(action)">{{ capabilities.isFavorite(action.id) ? '★' : '☆' }}</button>
          </article>
        }
      </section>
  ...
  styles: [`
    ...
    .section-head { margin:42px 0 15px; display:flex; justify-content:space-between; gap:20px; align-items:end; }.section-head h2 { margin:0; font-size:24px; letter-spacing:-.035em; }.section-head > span { color:var(--text-faint); font-size:10px; }.action-grid { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:10px; }.action-card { position:relative; min-width:0; min-height:104px; padding:0; overflow:hidden; color:var(--text); transition:transform var(--transition),border-color var(--transition),background var(--transition); }.action-card:hover { transform:translateY(-2px); border-color:color-mix(in srgb,var(--accent) 26%,var(--border)); }.action-launch { width:100%; min-height:102px; display:grid; grid-template-columns:42px minmax(0,1fr); grid-template-rows:1fr auto; gap:8px 11px; padding:14px 42px 14px 14px; border:0; background:transparent; color:inherit; text-align:left; }.favorite-button { position:absolute; top:10px; right:10px; width:27px; height:27px; display:grid; place-items:center; border:0; border-radius:8px; background:transparent; color:var(--text-faint); font-size:16px; cursor:pointer; transition:background var(--transition),color var(--transition),transform var(--transition); }.favorite-button:hover { background:var(--surface-2); color:var(--warning); transform:scale(1.06); }.favorite-button.active { color:var(--warning); background:var(--warning-soft); }.action-mark { grid-row:1/3; width:40px; height:40px; display:grid; place-items:center; border-radius:12px; background:var(--accent-soft); color:var(--accent); font-size:11px; font-weight:900; }.action-mark[data-category='pdf'] { background:#fff0ef; color:#c14f46; }.action-mark[data-category='image'] { background:#eaf6ff; color:#2875bb; }.action-mark[data-category='archive'] { background:var(--warning-soft); color:var(--warning); }.action-mark[data-category='privacy'] { background:var(--success-soft); color:var(--success); }.action-content { min-width:0; }.action-content strong,.action-content small { display:block; }.action-content strong { font-size:12px; letter-spacing:-.015em; }.action-content small { margin-top:4px; display:-webkit-box; overflow:hidden; color:var(--text-muted); font-size:10px; line-height:1.4; -webkit-box-orient:vertical; -webkit-line-clamp:2; }.action-state { color:var(--success); font-size:9px; font-weight:850; text-transform:uppercase; letter-spacing:.06em; }.action-state.missing { color:var(--warning); }
...
export class HomePage {
  ...
  protected async toggleFavorite(action: ActionDescriptor): Promise<void> {
    try { await this.capabilities.toggleFavorite(action.id); } catch { /* optimistic state is rolled back by the store */ }
  }
